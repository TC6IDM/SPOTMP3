...
class YouTubeDownloader(BaseDownloader):
    def download(self, link: str) -> Tuple[int, Path]:
        ...
        errors_file = self.errors_dir / f"ytdlp-{datetime.now().strftime('%Y%m%d%H%M%S')}.txt"
        
        cmd = [
            "yt-dlp",
            "--extract-audio", "--audio-format", "mp3",  # Audio → MP3
            ...
            # Note: Add --no-overwrites or sponsorblock if needed
            "--output", "%(playlist_title)s/%(playlist_index)02d %(uploader)s - %(title)s.%(ext)s",
            link,
        ]
        
        self.logger.info(f"📺 yt-dlp: {link.split('?')[0]}")
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                self.logger.info("✅ yt-dlp complete")
            else:
                self.logger.warning(f"yt-dlp exit: {proc.returncode}")
            return proc.returncode, errors_file
        except subprocess.TimeoutExpired:
            self.logger.error("⏰ yt-dlp timeout (2h)")
    ...
    def _cleanup_playlist(self, playlist_dir: Path, metadata_root: Path) -> List[Song]:
        """Private: cleanup one playlist (your cleanup_ytdlp_metadata logic)."""
